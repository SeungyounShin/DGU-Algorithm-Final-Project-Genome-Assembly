...
vector<vector<int>> filltable(int k, int n, vector<vector<int>> table) {
	...
	for (int i = 0; i < reference.size(); i++) {
		for (int j = 0; j < k / (MISSMATCH + 1); j++) {
			sumofascii += int(reference[i+j]);
		}
		table[sumofascii].push_back(i);
...
void perfectMatch(int k, int n, vector<vector<int>> table){
	...
	for (int i = 0; i < n; i++) { //shortread 개수만큼
		...
		isExit = 0;
		for (int j = 0; j < 3; j++) { //partition 만큼 반복
			tmp = 0;
			...
			for (int l = 0; l < k / (MISSMATCH + 1); l++) { //partition 길이만큼 반복
				...
				for (int m = 0; m < table[tmp].size(); m++) { //테이블 index 개수만큼
					...
					if (cnt == k / (MISSMATCH + 1)) { //만약 partition이 perfect match면 
						cout << shortread<<endl;
						for (int o = 0; o < k / (MISSMATCH + 1); o++) { // 복원될 string에 넣음
...
void MyDNA(string s, char Nucleic[], int k, int n) { //k 마다 1개씩 다르도록 sequence 생성
	...
	uniform_int_distribution<> dis(0, s.length() - 1); //0부터 3으로 분포 설정
...
int main() {
	...
	start = clock();
	table=filltable(30, 200,table);
	perfectMatch(30, 200,table);
	end = clock();
	cout << "복원하는 데 걸리는 시간 : " << (double)(end - start) / CLOCKS_PER_SEC << "s" << "\n" << endl;
	Result(30, 200);
	return 0;
